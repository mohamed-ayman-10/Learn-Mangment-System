        ...
        <div class="my-auto">
            <div class="d-flex">
                <h4 class="content-title mb-0 my-auto">@lang('fees.fees')</h4><span class="text-muted mt-1 tx-13 mr-2 mb-0">/
                    @lang('fees.ufs') {{ $student->name }}</span>
            </div>
        </div>
    </div>
    <!-- breadcrumb -->
...
@section('content')
    <!-- row -->
    <form action="{{ route('fee_invoices.update', $feeInvoices[0]->id) }}" method="post">
        @csrf
        @method('PUT')
        <input type="hidden" name="id" value="{{ $feeInvoices[0]->id }}">
        <input type="hidden" name="student_id" value="{{ $student->id }}">
        <input type="hidden" name="old_amount" value="{{ $feeInvoices[0]->amount }}">
        @if ($errors->any())
        ...
        <div class="row">
            <div class="form-group col-3">
                ...
                <input type="text" disabled name="name" class="form-control" value="{{ $student->name }}">
            </div>
            <div class="form-group col-3">
                ...
                <select name="type" required class="form-control">
                    ...
                    @foreach ($fees as $fee)
                        <option {{ $feeInvoices[0]->fees_id == $fee->id ? 'selected' : '' }} value="{{ $fee->id }}">
                            {{ $fee->title }}</option>
                    @endforeach
                </select>
            </div>
            <div class="form-group col-3">
                ...
                <input type="number" name="amount" value="{{ $feeInvoices[0]->amount }}" class="form-control" required
                    readonly>
            </div>
            <div class="form-group col-3">
                <label class="form-label">@lang('tables.notes')</label>
                <input type="text" name="desc" value="{{ $feeInvoices[0]->description }}" class="form-control">
            </div>
        </div>
