...
@section('content')
    ...
    <div class="row">
        ...
        @endif
        <form action="{{ route('processings.update', $processing->id) }}" method="post" class="w-100">
            @csrf
            @method('put')
            <div class="form-group">
                <label class="form-label" for="name">@lang('students.students_name')</label>
                <input type="text" value="{{ $processing->student->name }}" class="form-control" id="name" readonly
                    required placeholder="@lang('students.students_name')">
            </div>
            <div class="form-group">
                <label class="form-label" for="amount">@lang('fees.salary')</label>
                <input type="number" value="{{ $processing->amount }}" class="form-control" id="amount" name="amount"
                    required placeholder="@lang('fees.salary')">
            </div>
            <div class="form-group">
                <label class="form-label" for="desc">@lang('tables.notes')</label>
                <textarea name="desc" id="desc" class="form-control">{{ $processing->description }}</textarea>
            </div>
