                            ...
                            <tbody>
                                @foreach ($processings as $key => $processing)
                                    <tr>
                                        ...
                                        <td>
                                            <a href="{{ route('processings.edit', $processing->id) }}"
                                                class="btn btn-primary">
                                                <i class="typcn typcn-edit"></i>
                                            </a>
                                            <a href="#delete{{ $processing->id }}" data-effect="effect-scale"
                                    ...
                                    <div class="modal" id="delete{{ $processing->id }}">
                                        <div class="modal-dialog modal-dialog-centered" role="document">
                                            <div class="modal-content modal-content-demo">
                                                <div class="modal-header">
                                                    <h6 class="modal-title">{{ __('receipts.delete_excluded') }}
                                                    </h6><button aria-label="Close" class="close" data-dismiss="modal"
                                                        type="button"><span aria-hidden="true">&times;</span></button>
                                                </div>
                                                <form action="{{ route('processings.destroy', $processing->id) }}"
                                                    method="POST">
                                                    {{ method_field('DELETE') }}
                                                    @csrf
                                                    <div class="modal-body">
                                                        <p>{{ __('tables.delete_q') . ' ' . $processing->student->name . '؟' }}
                                                        </p>
                                                    </div>
