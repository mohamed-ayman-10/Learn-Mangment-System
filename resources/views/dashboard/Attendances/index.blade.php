        ...
        <div class="my-auto">
            <div class="d-flex">
                <h4 class="content-title mb-0 my-auto">@lang('attendances.attendance')</h4><span class="text-muted mt-1 tx-13 mr-2 mb-0">/
                    @lang('attendances.student_list')</span>
            </div>
        </div>
    </div>
    <!-- breadcrumb -->
                ...
                <div class="card-body">
                    <div class="table-responsive">
                        <form action="{{ route('attendances.store') }}" method="post">
                                            ...
                                            <td>
                                                @if (isset(
                                                    ...
                                                    <div class="btn text-success d-inline ">
                                                        <label for="true{{ $student->id }}">@lang('attendances.presence')</label>
                                                        <input type="radio" name="attendance[{{ $student->id }}]"
                                                            id="true{{ $student->id }}" disabled
                                                            {{ $student->attendances()->first()->status == 1 ? 'checked' : '' }}>
                                                    </div>
                                                    <div class="btn text-danger d-inline ">
                                                        <label for="false{{ $student->id }}">@lang('attendances.absence')</label>
                                                        <input type="radio" name="attendance[{{ $student->id }}]"
                                                            id="false{{ $student->id }}" disabled
                            ...
                            <button class="btn btn-primary mb-4" type="submit">@lang('tables.save')</button>
                        </form>
                    </div>
                </div>
            </div>
